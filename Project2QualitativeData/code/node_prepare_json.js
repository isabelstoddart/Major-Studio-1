...
// our search term
const search =  `name:"Newman, Carl" AND topic:"Nudity" AND online_media_type:"Images"`;
...
function fetchUrl(searchAllURL){
  request(searchAllURL, function (error, response, body) {
    ...
    let objects = obj.response.rows.filter(data => {
      return data.content.descriptiveNonRepeating.online_media.media[0].resources != undefined
    }).map((data) => {
