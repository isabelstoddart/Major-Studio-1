...
function downloadData() {
  fs.readFile("./data.json", "utf8", (err, data) => {
    if (err) console.log(err);
    JSON.parse(data).forEach(e => {
      if (!(e.title.includes("Standing") || e.title.includes("Seated") || e.title.includes("Sitting") || e.title.includes("Group") || e.title.includes("Composite") || e.title.includes("Two") || e.title.includes("Three") || e.title.includes("Four") || e.title.includes("Reclining") || e.title.includes("Back View"))){
        console.log('Downloading ' + e.filename);
      setTimeout(function() {
        downloadImage(e.primaryImage, e.filename, function(){
          console.log('Finished Downloading ' + e.filename);
