...
const search =  `Woman AND unit_code:"NPG" AND online_media_type:"Images"`;
...
function addObject(objectData) {  
  ...
  let date = "";
  if(objectData.content.freetext.date[0].content) {
...
function count(data) {
  ...
  data.forEach(data => {
    ...
    for(i=0;i < data.topic.length;i++){
      if(keys.includes(data.topic[i])) {
        frequencyCount[data.topic[i]] += 1;
      } else {
        frequencyCount[data.topic[i]] = 1;
      }
  }
